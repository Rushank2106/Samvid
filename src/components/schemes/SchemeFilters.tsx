  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-5 mb-8">
      ...
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full text-xs sm:text-sm pl-11 pr-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-jan-500 focus:outline-none bg-slate-50/50"
          />
          {searchQuery && (
            <button
              onClick={() => onSearchChange('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-400 hover:text-slate-600 bg-slate-200 px-2 py-0.5 rounded"
            >
              Clear
            </button>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t border-slate-100">
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-2 flex items-center gap-1.5">
            <Building2 className="w-3.5 h-3.5 text-jan-600" />
            {t.filter_gov_level}
          </label>
              ...
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition-all text-center ${
                  selectedGovLevel === level
                    ? 'bg-white text-jan-700 shadow-sm font-bold'
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-2 flex items-center gap-1.5">
            <MapPin className="w-3.5 h-3.5 text-jan-600" />
            {t.filter_state} Scope
          </label>
          <select
            value={selectedState}
            onChange={(e) => onSelectState(e.target.value)}
            className="w-full text-xs font-medium border border-slate-300 rounded-xl p-2.5 bg-white focus:ring-2 focus:ring-jan-500 focus:outline-none"
          >
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 mb-2 flex items-center gap-1.5">
            <SlidersHorizontal className="w-3.5 h-3.5 text-jan-600" />
            {t.filter_category}
          </label>
          <select
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pt-2 pb-1 no-scrollbar text-xs">
        <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1 pr-2 flex-shrink-0">
        ...
        {['All', 'Agriculture', 'Healthcare', 'Education', 'Housing', 'Pension', 'Women and Child Welfare'].map((c) => (
          <button
            ...
            className={`px-3 py-1 rounded-full text-xs font-medium flex-shrink-0 transition-colors ${
              selectedCategory === c
                ? 'bg-jan-700 text-white font-semibold'
